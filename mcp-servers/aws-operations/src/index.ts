    ...
    switch (name) {
      case 'get_stack_status': {
        const command = new DescribeStacksCommand({
          StackName: args.stackName as string,
        });
      ...
      case 'list_fod_stacks': {
        const env = (args.environment as string) || 'dev';
        ...
        const response = await cfnClient.send(command);
        const fodStacks = response.StackSummaries?.filter((s) =>
          s.StackName?.startsWith(`FOD-`) && s.StackName?.endsWith(`-${env}`)
        );
      ...
      case 'get_lambda_metrics': {
        ...
        const startTime = new Date(
          endTime.getTime() - (args.hours as number || 1) * 60 * 60 * 1000
        );
        ...
        const command = new GetMetricStatisticsCommand({
          ...
          Dimensions: [
            {
              Name: 'FunctionName',
              Value: args.functionName as string,
            },
          ],
          StartTime: startTime,
          EndTime: endTime,
      ...
      case 'list_lambda_functions': {
        ...
        const fodFunctions = response.Functions?.filter((f) =>
          f.FunctionName?.startsWith('FOD-')
        );
      ...
      case 'get_deployment_events': {
        ...
        const response = await cfnClient.send(command);
        const events = response.StackEvents?.slice(0, args.limit as number || 20);
